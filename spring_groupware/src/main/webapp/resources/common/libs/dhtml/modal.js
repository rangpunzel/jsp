...
open:function(t, contenttype, contentsource, title, attr, recalonload){
	...
	this.loadveil(d.zIndexvalue)
	if (recalonload=="recal" && d.scroll_top==0)
		d.addEvent(window, function(){dhtmlmodal.adjustveil()}, "load")
	var t=d.open(t, contenttype, contentsource, title, attr, recalonload)
//	t.controls.firstChild.style.display="none" //Disable "minimize" button
	$(t).find("[title=Minimize]").hide();
	$(t).find("[title=openwin]").hide();
...
loadveil:function(zIndex){
	...
	if (this.interVeil.style.zIndex)
		this.interVeil.style.zIndex=zIndex // 모달창인 경우 부모창이 있는경우에 부모창까지도 접근을 막습니다. 2013-03-11 LSH
},
