...
function ShowUserInfo(event, userId) {
	...
	var url = "/commons/userInfo?id=" + userId ;
	...
	xmlHttpRequest.onreadystatechange = function(){
		if ( xmlHttpRequest.readyState == 4 ) {
			...
			ModalDialog({'t':'임직원 프로파일', 'tp':winx, 'lp':winy, 'w':400, 'h':210, 'm':'html', 'c':data, 'modal':false, 'd':false, 'r':false });
			/*
...
function ShowUserInfoSet() {
     ...
     var strUrl = "/commons/userInfo?id=";
   	...
   	$('.maninfo').each(function()
    {
   		...
   		$(this).qtip(
   		{
   			content: {
   				...
   				title: {
   					text: '임직원 프로필 - ID : ' + $(this).text(), // Give the tooltip a title using each elements text
...
Date.prototype.format = function (f) {
    ...
    return f.replace(/(yyyy|yy|MM|dd|KS|KL|ES|EL|HH|hh|mm|ss|a\/p)/gi, function ($1) {
        switch ($1) {
            ...
            case "yy": return (d.getFullYear() % 100).zf(2); // 년 (2자리)
